...
Features:
  ...
  - Stats Screen
  - yt-dpl migration from pytubefix
"""
...
def bulk_download_songs(songs: List[Dict], base_path: str, org_choice: str,
    ...
    for i, song in enumerate(songs, 1):
        ...
        # ── Download ──────────────────────────────────────────────────────────
        try:
            yt = YouTube(video.watch_url, use_oauth=True, client='ANDROID_TV')
...
def import_playlist(spotify_mgr: SpotifyManager, base_path: str) -> None:
    ...
    for i, song in enumerate(songs, 1):
        ...
        try:
            ...
            if search.videos:
                yt = YouTube(search.videos[0].watch_url, use_oauth=True, client='ANDROID_TV')
...
def import_top_tracks(spotify_mgr: SpotifyManager, base_path: str) -> None:
...
def download_artist_top_tracks(spotify_mgr: SpotifyManager, base_path: str) -> None:
...
def download_artist_albums(spotify_mgr: SpotifyManager, base_path: str) -> None:
...
def import_followed_artists(spotify_mgr: SpotifyManager, base_path: str) -> None:
    ...
    if choice == "b":
        ...
        download_all_artists_top_songs(spotify_mgr, artists, base_path)
    elif choice == "2":
        # Download albums for all artists
        download_all_artists_albums(spotify_mgr, artists, base_path)
    elif choice == "3":
        # Select specific artist
        download_specific_followed_artist(spotify_mgr, artists, base_path)


def download_all_artists_top_songs(spotify_mgr: SpotifyManager, artists: List[Dict], base_path: str) -> None:
    ...
    for i, artist in enumerate(artists, 1):
        ...
        try:
            query = f"{artist['name']} greatest hits"
            search = Search(query)
            
            for j, video in enumerate(search.videos[:5], 1):  # Top 5 songs per artist
                try:
                    console.print(f"  [dim]{j}/5: {video.title[:50]}[/dim]")
                    yt = YouTube(video.watch_url, use_oauth=True, client='ANDROID_TV')
                    download_audio_format(yt, artist_folder, format_choice)
                except Exception as e:
                    console.print(f"  [danger]Skipped: {e}[/danger]")
    ...
    console.print(f"\n[success]✔ Completed download for all {len(artists)} artists![/success]")


def download_all_artists_albums(spotify_mgr: SpotifyManager, artists: List[Dict], base_path: str) -> None:
    ...
    for i, artist in enumerate(artists, 1):
        ...
        try:
            ...
            albums = spotify_mgr.get_artist_albums(artist['id'])
            ...
            # Download each album
            for album in albums[:3]:  # Limit to 3 most recent albums
                ...
                album_folder = a_dir / safe_artist / safe_album
                ...
                for track in tracks:
                    try:
                        search = Search(track['search_query'])
                        if search.videos:
                            yt = YouTube(search.videos[0].watch_url, use_oauth=True, client='ANDROID_TV')
                            download_audio_format(yt, album_folder, format_choice)
                    except Exception as e:
                        console.print(f"    [danger]Skipped {track['name']}: {e}[/danger]")
...
def download_specific_followed_artist(spotify_mgr: SpotifyManager, artists: List[Dict], base_path: str) -> None:
    ...
    if choice == "b":
        ...
        download_all_artists_top_songs(spotify_mgr, [artist], base_path)
    elif choice == "2":
        download_all_artists_albums(spotify_mgr, [artist], base_path)
    ...
    try:
        if is_list:
            ...
            with console.status("[info]Loading metadata...[/info]"):
                yt = YouTube(url, use_oauth=True, client='ANDROID_TV')
